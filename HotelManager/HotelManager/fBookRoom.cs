...
namespace HotelManager
{
    public partial class fBookRoom : Form
    {
        ...
        private void LoadRoomTypeInfo(int id)
        {
            RoomTypeDTO roomType = RoomTypeDAO.Instance.LoadRoomTypeInfo(id);
        ...
        {
            dpkDateOfBirth.Value = new DateTime(2018, 4, 6);
        ...
        {
            LoadRoomTypeInfo((cbRoomType.SelectedItem as RoomTypeDTO).Id);
        }
        ...
        private void btnBookRoom_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bạn có muốn đặt phòng không?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                if (txbIDCard.Text != String.Empty && txbFullName.Text != String.Empty && txbAddress.Text != String.Empty && txbPhoneNumber.Text != String.Empty && cbNationality.Text != String.Empty)
                {
                    ...
                    InsertBookRoom(CustomerDAO.Instance.GetInfoByIdCard(txbIDCard.Text).Id, (cbRoomType.SelectedItem as RoomTypeDTO).Id, dpkDateCheckIn.Value, dpkDateCheckOut.Value, DateTime.Now);
